...
document.addEventListener("alpine:init", () => {
    a.data("helpComponent", () => ({
        ...
        supportTicket: {name: "", email: "", priority: "", category: "", subject: "", description: ""},
        submittingTicket: !1,
        featureRequest: {title: "", category: "", description: ""},
        sections: [{id: "getting-started", name: "Getting Started", icon: "bi-play-circle"}, {
            id: "faq",
            name: "FAQ",
        ...
        systemStatus: [{name: "API Services", status: "operational", statusText: "Operational"}, {
            name: "Database",
            status: "operational",
        ...
        init() {
            console.log("Help component initialized"), this.filteredFAQ = [...this.faqData], this.loadUserPreferences()
        },
        setActiveSection(e) {
            this.activeSection = e, e === "documentation" && !this.selectedDocCategory && (this.selectedDocCategory = "getting-started")
        },
        toggleFAQ(e) {
        ...
        filterFAQ() {
            ...
            this.filteredFAQ = this.faqData.filter(t => t.question.toLowerCase().includes(e) || t.answer.toLowerCase().includes(e)), this.filteredFAQ.forEach(t => t.open = !1)
        },
        highlightText(e, t) {
            if (!t) return e;
        ...
        submitTicket() {
            this.validateTicketForm() && (this.submittingTicket = !0, setTimeout(() => {
                this.submittingTicket = !1, this.showNotification("Support ticket submitted successfully! You will receive a confirmation email shortly.", "success"), this.resetTicketForm()
            }, 2e3))
        },
        validateTicketForm() {
            return ["name", "email", "priority", "category", "subject", "description"].filter(i => !this.supportTicket[i]).length > 0 ? (this.showNotification("Please fill in all required fields", "error"), !1) : !0
        },
        resetTicketForm() {
            this.supportTicket = {name: "", email: "", priority: "", category: "", subject: "", description: ""}
        },
        submitFeatureRequest() {
            ...
            this.popularRequests.unshift(e), this.showNotification("Feature request submitted successfully!", "success"), this.featureRequest = {
                title: "",
                category: "",
                description: ""
            }
        ...
        voteFeature(e) {
            ...
            t && !t.voted && (t.votes++, t.voted = !0, this.showNotification("Thank you for your vote!", "success"))
        },
        viewSystemStatus() {
        ...
        saveUserPreferences() {
            const e = {lastSection: this.activeSection, savedAt: new Date().toISOString()};
            localStorage.setItem("helpPreferences", JSON.stringify(e))
        },
        showNotification(e, t = "info") {
            console.log(`[${t.toUpperCase()}] ${e}`), document.dispatchEvent(new CustomEvent("showNotification", {
                detail: {
                    message: e,
                    type: t
                }
        ...
        query: "", results: [], search() {
            if (console.log("Searching help articles for:", this.query), this.query.length > 2) {
                this.results = [{
                    ...
                    url: "#contact",
                    type: "Help"
                }].filter(t => t.title.toLowerCase().includes(this.query.toLowerCase()));
                const e = a.$data(document.querySelector('[x-data="helpComponent"]'));
                e && e.activeSection === "faq" && (e.faqSearch = this.query, e.filterFAQ())
            } else this.results = []
        }
    })), a.data("themeSwitch", () => ({
